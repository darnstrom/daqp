...
void update_LDL_remove(DAQPWorkspace *work, const int rm_ind){
    if(work->n_active==rm_ind+1)
        return;
    int i, j, r, old_disp, new_disp, z_count , n_update=work->n_active-rm_ind-1;
    c_float* z = &work->zldl[rm_ind]; // zldl will be obsolete => use to allocations
    // Extract parts to keep/update in L & D
    new_disp=ARSUM(rm_ind);
    old_disp=new_disp+(rm_ind+1);
    z_count = 0;
    ...
    for(i = rm_ind+1;i<work->n_active;old_disp++,new_disp++,i++) //(disp++ skips blank element)..
        for(j=0;j<i;j++){ 
            if(j!=rm_ind)
                ...
                z[z_count++] = work->L[old_disp++];
        }
    // t-method in Fletcher 1974 for low-rank update of LDL
    c_float d,v,beta,t,tn,alpha,ztemp;
    t = 1/work->D[rm_ind];
    // i - Element/row to update|j - Column which is looped over|r - Row to loop over
    old_disp=ARSUM(rm_ind)+rm_ind;
    for(j = 0, i=rm_ind; j<n_update;j++,i++){
        ...
        if(d == 0){
            work->D[i] = v*v/t;
            break; // Assume singularity only in last element
        }
        else{
            tn= t+v*v/d;
            alpha = tn/t;
        ...
        if(alpha<4){
            for(r=j+1, new_disp=old_disp+j;r<n_update;r++){
                z[r] -= v*work->L[new_disp];
                work->L[new_disp]+= beta*z[r];
                new_disp+=rm_ind+r+1;
            }
        }
        else{
            alpha = 1/alpha; // called gamma in paper
            for(r=j+1, new_disp=old_disp+j;r<n_update;r++){
                ztemp = z[r];
                z[r] -= v*work->L[new_disp];
                work->L[new_disp] =alpha*work->L[new_disp]+beta*ztemp;
                new_disp+=rm_ind+r+1;
            }
        }
    }
