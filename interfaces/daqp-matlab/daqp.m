    ...
    methods
        ...
        function [exitflag,setup_time] = setup(this,H,f,A,bupper,blower,sense,break_points)
            ...
            [exitflag,setup_time] = daqpmex('setup', this.work_ptr,this.H,this.f,...
                this.A,this.bupper,this.blower,this.sense,this.bin_ids,this.break_points);
        end
        ...
        function settings = settings(this,varargin)
            ...
            if(length(varargin)==1)
                if(isstruct(varargin{1})) % If struct as input treat it as new settings
                    ...
                    if(isstr(varargin{1})&& strcmp(varargin{1},'default'))
                        daqpmex('set_default_settings', this.work_ptr);
                        settings = daqpmex('get_settings', this.work_ptr);
                        return
            ...
            for k=1:numel(fn)
                if(isfield(settings,fn{k}) && isscalar(new_settings.(fn{k}))) 
                    settings = setfield(settings,fn{k},cast(new_settings.(fn{k}),...
                        class(settings.(fn{k}))));
                else
